...
import { PROJECT_CONFIG_FILENAME, writeProjectConfig } from '../utils/project-config.js';
...
export async function initCommand(options: InitOptions = {}) {
  ...
  const filesToCreate = [
    ...(database === 'chdb'
      ? [path.join(process.cwd(), PROJECT_CONFIG_FILENAME)]
      : []),
  ...
  if (database === 'chdb') {
    ...
    logger.success(`Created ${PROJECT_CONFIG_FILENAME}`);
  }
  ...
  if (hasValidConnection) {
    ...
    const regenerateHint = database === 'chdb'
      ? "// Run 'npx hypequery generate' after creating tables"
...
`);
  ...
  if (hasValidConnection) {
    ...
    logger.indent('2. Run: npx hypequery generate');
  ...
  if (database === 'chdb' && hasValidConnection) {
    logger.indent('hypequery generate          Refresh types after creating tables');
    logger.newline();
  }
