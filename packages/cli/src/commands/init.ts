...
function setConnectionEnv(config: ConnectionConfig): void {
  process.env.CLICKHOUSE_HOST = config.host;
  process.env.CLICKHOUSE_DATABASE = config.database;
  process.env.CLICKHOUSE_USERNAME = config.username;
...
async function testConnection(
  ...
  setConnectionEnv(connectionConfig);
...
export async function initCommand(options: InitOptions = {}) {
  ...
  if (hasValidConnection) {
    ...
    await writeFile(schemaPath, generatePlaceholderSchemaTemplate());
