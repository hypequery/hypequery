...
describe('plan command', () => {
  ...
  it('writes preview artifacts without writing a migration or snapshot', async () => {
    ...
    await planCommand({ name: 'add users' });

    expect(fs.writeFile).toHaveBeenCalledTimes(4);
    expect(vi.mocked(fs.writeFile)).toHaveBeenCalledWith(
      expect.stringContaining('migrations/meta/plan-preview/up.sql'),
      expect.stringContaining('CREATE TABLE users;'),
      'utf8',
    );
    expect(vi.mocked(fs.writeFile)).toHaveBeenCalledWith(
      expect.stringContaining('migrations/meta/plan-preview/plan.json'),
      expect.stringContaining('"operations"'),
      'utf8',
    );
  });

  it('exits when rendering fails for a blocked plan', async () => {
    ...
    await expect(planCommand({ name: 'drop users' })).rejects.toBeInstanceOf(ProcessExitError);
    exitHandler.restore();
  });
});
