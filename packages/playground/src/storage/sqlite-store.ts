...
// Structural type for node:sqlite's DatabaseSync (avoid static import so the
// module can load — and fall back to MemoryStore — on Node < 22.5).
 ...
 * dependency-free SQLite operations. Ideal for local development.
 *
 * Note: `node:sqlite` is dynamically imported to allow graceful fallback
 * to in-memory storage on older Node versions.
 */
export class SQLiteStore implements QueryHistoryStore {
   ...
   * @throws Error if `node:sqlite` is unavailable (Node < 22.5)
   */
  async initialize(): Promise<void> {
    // Dynamic import to allow graceful fallback on Node versions without node:sqlite
    const { DatabaseSync } = await import('node:sqlite');
    this.db = new DatabaseSync(this.dbPath) as unknown as NodeSqliteDatabase;
  ...
  async batchInsert(logs: Array<QueryLog & { queryId: string }>): Promise<void> {
    ...
    this.db.exec('BEGIN');
    try {
      for (const log of logs as Array<QueryLog & { queryId: string; endpointKey?: string; endpointDescription?: string; endpointPath?: string }>) {
        stmt.run(
          log.queryId,
