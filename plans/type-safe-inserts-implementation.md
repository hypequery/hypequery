...
7. **`Date`/`Date32` columns only parse bare `'YYYY-MM-DD'` strings in JSONEachRow** — `date_time_input_format: 'best_effort'` applies to DateTime/DateTime64 only, and the Date parser rejects anything after the date part (verified against ClickHouse 25.1 during integration testing; ISO strings with `T...Z` fail with `CANNOT_PARSE_INPUT_ASSERTION_FAILED`). Since the schema is phantom at runtime, normalization cannot format `Date` objects per-column-type, so the insert types accept `string` only for Date/Date32 columns — what compiles is what works. Epoch-second numbers into DateTime columns were also verified to work.
