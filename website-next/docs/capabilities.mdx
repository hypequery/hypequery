...
| CTEs | Native | `withCTE()` with a builder or SQL body; the alias is a typed join target when its columns are known, and `db.withCTE(...).table(alias)` reads from it |
| Recursive CTEs | Native | `withRecursiveCTE()` renders `WITH RECURSIVE` from a raw body with `{name:Type}` parameters |
