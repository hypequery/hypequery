...
| CTEs | Native | `withCTE()` declares a CTE for typed joins or `.table(alias)` queries |
| Recursive CTEs | Native | `withRecursiveCTE()` supports raw SQL with `{name:Type}` parameters |
...
| `approxCountDistinct(column, alias?)` | estimated distinct count with `uniq` |
...
| Minute, hour, day, week, month, quarter, and year grains | Shipped (minute and hour are local only until Cloud supports them) |
