...
Installing `chdb` does not prevent the CLI from connecting to ClickHouse Cloud or another remote server. `CLICKHOUSE_*` environment variables and `.env` configuration take precedence over dependency-based chDB detection. Without remote configuration or a `chdb` dependency, the CLI defaults to the ClickHouse driver. You can also select the remote driver explicitly:
