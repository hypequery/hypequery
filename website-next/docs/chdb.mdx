...
Installing `chdb` does not prevent the CLI from connecting to ClickHouse Cloud or another remote server. `CLICKHOUSE_*` environment variables and `.env` configuration take precedence over dependency-based chDB detection. You can also select the remote driver explicitly:
