...
`hypequery init` can scaffold a project straight onto the embedded engine — no server, no credentials, no `.env`:
...
The scaffolded `client.ts` uses the adapter instead of an HTTP connection, `chdb` is installed as a dependency automatically, and the connection prompts are skipped. Choose an on-disk session directory (e.g. `./analytics.chdb`) to persist data between runs, or stay in-memory for a throwaway sandbox. The choice is saved in `hypequery.config.json`, so after creating tables a plain generate command refreshes types from the same session:
...
See the [CLI reference](/docs/reference/api/cli) for all `init` and `generate` flags.
