...
The scaffolded `client.ts` uses the adapter instead of an HTTP connection, `chdb` is installed as a dependency automatically, and the credential prompts are skipped. Choose an on-disk session directory (e.g. `./analytics.chdb`) to persist data between runs, or stay in-memory for a throwaway runtime sandbox. The choice is saved in `hypequery.config.json`. With on-disk storage, a plain generate command reopens that directory and refreshes types after you create tables:
