...
<Callout type="warn" title="Soon to be deprecated for new definitions">
  Prefer [dataset-owned measures](/docs/datasets/measures#derived-measures),
  including `measure.derived(...)` in the same `measures` object, for new
  calculations. Standalone metrics still work locally and with self-hosted
  Serve. This page documents the existing API for applications that still use it.
</Callout>
...
Both paths use the same dataset definition and validation rules. Prefer dataset
queries with named base or derived measures for new work; use the standalone
metric path when maintaining an existing local or self-hosted integration.
...
Supported grains are `minute`, `hour`, `day`, `week`, `month`, `quarter`, and `year`. See [time grains](/docs/datasets/time-grains) for restricting them per dataset.
...
For the normalized metadata shape used by MCP introspection, see [Catalog](/docs/datasets/catalog).
