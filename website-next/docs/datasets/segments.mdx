...
A dataset that declares segments accepts `segments` in its Serve endpoint, its OpenAPI schema, the MCP `query_dataset` and `query_metric` tools, and the React hooks. The input schema lists the segment names as an enum, so an unknown name is rejected before the query runs. Datasets without segments do not accept the field, so their schemas are unchanged.
