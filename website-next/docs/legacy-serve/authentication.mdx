...
const { useAuth, TypedAuth } = createAuthSystem({
  roles: ['admin', 'super-admin', 'editor', 'viewer'] as const,
...
const authStrategy = async ({ request }): Promise<AppAuth | null> => {
  // ServeRequest.headers is a plain object, not a Fetch Headers instance.
  ...
  queries: {
    ...
    writeData: query
      ...
      .query(async ({ ctx, input }) => ({
        success: true,
        data: await ctx.db.table('data').insert(input.data),
      })),
...
<Callout type="info" title="ServeRequest headers are plain objects">
  Auth strategies receive a <code>ServeRequest</code> with <code>headers</code> as a simple
  object (<code>Record&lt;string, string | undefined&gt;</code>). Use
  <code>request.headers['x-auth-token']</code> or <code>request.headers.authorization</code> rather than
  <code>request.headers.get()</code>. For case-insensitive + array-safe access, use
  <code>getHeader</code> from <code>@hypequery/serve</code>.
</Callout>
...
`.require()` is an alias if you prefer a shorter chain.
...
For header-based auth, use `getHeader` (case-insensitive, array-safe) or the `apiKeyAuth` adapter for the common API key flow:

<CodeBlock>
```typescript
import { apiKeyAuth, getHeader } from '@hypequery/serve';

// 80% case: API key auth with clear missing/invalid errors
const apiKeyStrategy = apiKeyAuth({
  header: 'x-tenant-key',
  validate: (key) => lookupTenant(key),
});

// If you need custom logic, use getHeader directly
const customHeaderStrategy = async ({ request }) => {
  const key = getHeader(request, 'x-tenant-key');
  if (!key) return null;
  return lookupTenant(key);
};
```
</CodeBlock>
...
<Callout type="warning" title="Internal calls still enforce auth">
`api.run()` follows the same auth pipeline as HTTP. For internal jobs, pass a trusted service token in headers.
</Callout>
...
const result = await api.run('adminOnly', {
  input: {},
  request: {
    headers: { 'x-auth-token': process.env.INTERNAL_SERVICE_TOKEN! },
  },
});
```
</CodeBlock>
