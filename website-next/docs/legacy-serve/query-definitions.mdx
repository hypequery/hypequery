---
title: Legacy Query Definitions
description: Legacy builder-first query definition patterns for the older hypequery serve API.
---
...
<Callout type="info">
  This page documents the legacy builder-first Serve API. For new projects, start with <a href="/docs/quick-start">Quick Start</a>. If you are migrating existing code, use <a href="/docs/migrate-legacy-serve-api">Migrate from Legacy Serve API</a>. For the current execution model, read <a href="/docs/core-concepts">Core Concepts</a>.
</Callout>
...
Per-query tenant configs merge with the global `defineServe` tenant config, so you can override just what you need:

<CodeBlock>
```typescript
tenant: { required: false } // optional tenant for this query only
```
</CodeBlock>
...
Use `InferQueryResult` when you trust the builder's static typing (no schema required). `InferQueryOutput` reads the optional `outputSchema`, which is handy when runtime validation is the source of truth. Both helpers accept either the `define()` result (your exported `api`) or a raw `ServeQueriesMap`, so you can infer types from any subset of queries.

### Inline Execution Helpers

Calling `await api.run('tripsQuery')` (alias of `api.execute` and `api.client`) now returns the same type as `InferQueryResult<typeof api, 'tripsQuery'>`. For inline scripts or devtools, you can lean on the helper to annotate external call sites:
