...
description: Low-level and legacy builder-first serve reference.
---
...
# Legacy `@hypequery/serve` reference

<Callout type="info">
  This reference page covers the full legacy and low-level Serve surface. For the recommended happy path, start with <a href="/docs/quick-start">Quick Start</a>. For the current execution model, read <a href="/docs/core-concepts">Core Concepts</a>.
</Callout>

The serve package turns your metrics into HTTP endpoints, docs, and OpenAPI artifacts. This page lists every exported primitive so you can wire the runtime intentionally. For the conceptual overview, read [Core Concepts](/docs/core-concepts) first.
...
<TypeTable
  type={{
    ...
    security: {
      type: 'SecurityConfig',
      default: 'undefined',
      description: 'Security hardening options. See [Security hardening](#security-hardening) below.',
    },
    cors: {
      type: 'boolean | CorsConfig',
      default: 'undefined (disabled)',
      description: 'Cross-origin resource sharing policy. `true` for permissive defaults (allow all origins), or a `CorsConfig` object for fine-grained control. Handles preflight automatically.',
    },
    queryLogging: {
...
<TypeTable
  type={{
    origin: {
      ...
      default: '"*"',
      description: 'Allowed origin(s). Pass `"*"` to allow all, a string for an exact match, an array to allowlist multiple origins, or a function for custom logic. When `credentials` is `true`, the request origin is always echoed rather than `"*"`.',
    },
    methods: {
      type: 'string[]',
      default: '["GET","POST","PUT","PATCH","DELETE","OPTIONS"]',
      description: 'HTTP methods exposed to cross-origin requests.',
    },
    allowedHeaders: {
      type: 'string[]',
    ...
    exposedHeaders: {
      type: 'string[]',
      default: '[]',
      description: 'Response headers the browser is allowed to read.',
    },
    credentials: {
      type: 'boolean',
      default: 'false',
    ...
    maxAge: {
      type: 'number',
      default: '86400',
      description: 'How long (in seconds) the browser should cache preflight responses.',
    },
  }}
/>
...
<TypeTable
  type={{
    ...
    max: {
      type: 'number',
      default: '100',
      description: 'Maximum requests allowed per window per key.',
    },
    keyBy: {
      type: '(ctx: EndpointContext) => string | null',
      default: 'IP from x-forwarded-for / x-real-ip',
      description: 'Function to derive the rate limit key. Return `null` to skip limiting for a request.',
    },
    store: {
      type: 'RateLimitStore',
      default: 'MemoryRateLimitStore',
      description: 'Backing store for counters. The default in-memory store is suitable for single-process deployments. Implement `RateLimitStore` for Redis or other distributed stores.',
    },
    headers: {
      type: 'boolean',
      default: 'true',
      description: 'Whether to include `X-RateLimit-*` headers in responses.',
    },
    message: {
      type: 'string',
      default: '"Too many requests, please try again later"',
      description: 'Message included in the `429` error body.',
    },
    failOpen: {
      type: 'boolean',
      default: 'true',
      description: 'If the store throws, pass the request through instead of rejecting it.',
    },
  }}
/>
...
<TypeTable
  type={{
    bodyLimit: {
      ...
      description: 'Maximum request body size in bytes. Requests exceeding this limit are rejected with `413 Payload Too Large`. Set to `0` to disable.',
    },
    requestTimeout: {
      type: 'number',
      default: '30000 (30 s)',
      description: 'Maximum time in milliseconds a handler may run before the server returns `504 Gateway Timeout`. The underlying handler continues running but its response is discarded. Set to `0` to disable.',
    },
    gracefulShutdownTimeout: {
      type: 'number',
      default: '10000 (10 s)',
      description: 'After a shutdown signal, the server stops accepting new connections and waits up to this many milliseconds for in-flight requests to complete. Any remaining connections are force-closed after the deadline.',
    },
  }}
/>
