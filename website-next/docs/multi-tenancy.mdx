...
<Callout type="info" title="tenantOptional requires extract when no global tenant config exists">
  If you call <code>tenantOptional()</code> without a global tenant config, you still need to provide
  <code>extract</code> in the per-query override.
</Callout>
