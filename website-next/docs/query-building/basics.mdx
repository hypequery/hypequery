...
<Callout type="info" title="No server required">
  Everything on this page also runs on embedded ClickHouse via [chDB](/docs/chdb) — pass
  `{ adapter: chdbAdapter({ session }) }` to `createQueryBuilder` instead of a client and the
  same builder code executes in-process. Handy for tests and local development.
</Callout>
...
const rows = await query.select(['id', 'email']).execute();
```
</CodeBlock>

<CodeBlock>
```typescript
// Fragile: these calls do not update query unless you keep the return value
