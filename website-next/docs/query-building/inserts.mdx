...
The `insertInto()` method starts a type-safe insert. Row shapes are derived from your schema: `Nullable(...)` columns are optional, every other column is required, and value types are checked at compile time.
...
db.insertInto('events').values({ id: 1 });         // ✗ missing required columns
db.insertInto('events').values({ nope: 1, ... });  // ✗ unknown column
```
