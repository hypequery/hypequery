...
<Callout type="warning" title="v2.0.0 behavior">
  `withRelation()` is stricter now. Alias override is only supported for single-step relationships, and direct `JoinPath` usage is the typed path when you want compile-time widening.
</Callout>
...
Alias override is only supported for single-step relationships. For `defineChain()` relationships, define aliases on the chain steps themselves if needed.
