...
`limitBy()` applies ClickHouse `LIMIT ... BY ...`, which limits rows per grouping key instead of limiting the whole result set.

<CodeBlock>

```typescript
const latestOrderPerUser = await db.table('orders')
...
You can also limit by multiple columns:
...
const results = await db.table('orders')
  ...
  .orderBy('created_at', 'DESC')
  .limitBy(2, ['user_id', 'status'])
  .execute();
```

</CodeBlock>

`limitBy()` is applied before the final `limit()` if you use both.
...
const stats = await db.table('events')
  .select(['user_id'])
  .countDistinct('event_type', 'unique_events')
  .groupBy(['user_id'])
