...
Helpers like `toDateTime`, `formatDateTime`, `toStartOfInterval`, the built-in `toStartOf*` helpers, and `datePart` wrap common ClickHouse functions with type safety:
...
The built-in time-bucketing helpers are also exported directly from `@hypequery/clickhouse`, for example `toStartOfMinute`, `toStartOfHour`, `toStartOfDay`, `toStartOfWeek`, `toStartOfMonth`, `toStartOfQuarter`, and `toStartOfYear`.
