---
title: Quick Start: Serve Framework
description: Ship your first governed API endpoint in minutes
---
...
<Callout type="info">
  **New to Hypequery?** Start with the [Query Builder Quick Start](/docs/quick-start-builder) to learn the basics, then come here when you're ready to add HTTP APIs and team features.

  Not sure which to use? See [Choose Your Path](/docs/standalone-query-builder/when-to-use).
</Callout>
...
<Tabs items={['embedded', 'over HTTP', 'with React hooks']} defaultIndex={0}>
  <Tab value='embedded' label='embedded'>
  <CodeBlock>
    ```typescript
    // Run directly in your code (jobs, SSR, scripts)
    const latest = await api.run('activeUsers', { input: { limit: 25 } });
    console.log(latest);
    ```
    </CodeBlock>
  </Tab>
  ...
  <Tab value='with React hooks' label='with React hooks'>
  <CodeBlock>
    ```typescript
