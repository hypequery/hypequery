...
The `hypequery` CLI provides commands for scaffolding, generating types, and running development servers. Every command expects access to your ClickHouse credentials via the prompts presented during `init` or via the `CLICKHOUSE_*` environment variables listed below — except with `--database chdb`, which runs on [embedded ClickHouse](/docs/chdb) and needs no credentials at all.
...
- **`--database <type>`** – Database driver: `clickhouse` (default) or `chdb` to scaffold onto [embedded ClickHouse](/docs/chdb) — no server, no credentials, no `.env`
- **`--chdb-path <path>`** – With `--database chdb`, the directory the embedded session stores data in (e.g. `./analytics.chdb`); omit for an in-memory session. `init` saves this choice in `hypequery.config.json` for later generate commands
...
`hypequery generate` auto-detects your database configuration and ships with the ClickHouse driver, so there’s no extra package to install. Pass `--database chdb` to introspect an [embedded ClickHouse](/docs/chdb) session instead of a server. Projects created by `init --database chdb` reuse the driver and session path saved in `hypequery.config.json`; dependency-based detection remains as a fallback for existing chDB projects.
...
- **`--chdb-path <path>`** – With `--database chdb`, override the configured embedded session directory to introspect; omit when `hypequery.config.json` already contains the path or for an in-memory session
