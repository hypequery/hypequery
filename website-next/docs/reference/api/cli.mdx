...
The one exception is `hypequery init --auth context`, which scaffolds an `api.ts` with a trusted tenant runtime on `tenant_id`. Because you chose that tenant boundary explicitly, generated datasets declare `tenantKey: 'tenant_id'` to match it. Tables without a `tenant_id` column are reported as warnings — tenant-scoped requests against them fail until you set `tenantKey` to the right column.
