...
- **`--chdb-path <path>`** – With `--database chdb`, the directory the embedded session stores data in (e.g. `./analytics.chdb`); omit for an in-memory session
...
- **`--chdb-path <path>`** – With `--database chdb`, the embedded session directory to introspect (the same path your scaffolded `client.ts` uses); omit for in-memory
