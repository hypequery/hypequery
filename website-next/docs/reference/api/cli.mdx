...
The generator connects to ClickHouse with your `CLICKHOUSE_*` env vars and renders a `datasets` export containing a `dataset(...)` definition per table with inferred dimensions and measures. It creates a missing output file, but refuses to overwrite an existing customized file unless you pass `--force`; replacements are written atomically. Use `--check` in CI or `--diff` to review schema-driven changes without writing. Review and customize the output, then import it into your application or `api.ts`.
