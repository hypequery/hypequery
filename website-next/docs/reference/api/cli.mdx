...
Detection prefers remote ClickHouse configuration over an installed `chdb` dependency. The order is: `CLICKHOUSE_*` environment variables, ClickHouse variables in `.env`, then a `chdb` dependency. Use `--database clickhouse` or `--database chdb` to override detection explicitly. This means a project may depend on `chdb` and still generate against ClickHouse Cloud:
