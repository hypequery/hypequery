...
<Callout type="info" title="Prefer `url`">
...
You don't need a ClickHouse server at all: [chDB](https://github.com/chdb-io/chdb) runs the ClickHouse engine inside your Node process, and the chDB team maintains a hypequery adapter (`chdb/hypequery`). Same builder code, no server — ideal for local development, CI tests, and serverless:
...
Swap to remote ClickHouse for production by handing `createQueryBuilder` your connection details instead of the adapter — nothing else changes. See [chDB (Embedded ClickHouse)](/docs/chdb) for the full guide.

More generally, `createQueryBuilder({ adapter })` accepts any implementation of the [`DatabaseAdapter` contract](/docs/reference/custom-adapters), so the builder can run against any engine that speaks ClickHouse SQL.
