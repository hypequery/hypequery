...
<Callout type="info" title="Currently dormant">
  No ingest endpoint ships in the published packages, so telemetry is a complete no-op today—nothing is sent regardless of your settings. When an endpoint is configured in a future release, the first run that would send anything will print a one-time notice in your terminal. Telemetry is automatically disabled in CI.
</Callout>
...
```bash
hypequery dev --no-telemetry
```

This flag disables telemetry for that command, regardless of any other setting.
