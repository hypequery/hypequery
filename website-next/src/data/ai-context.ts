export const aiContext = `You are assisting with hypequery, a TypeScript-first analytics layer for ClickHouse.
...
// 2. Define reusable queries with object-style config
...
// 3. Add the runtime when you want routes/docs/handlers
export const api = serve({
  queries: { weeklyRevenue },
});

// 4. Execute in-process
...
// Hono/Edge
const edgeHandler = createFetchHandler(api.handler);
\`\`\`
...
- Start with \`db\` when the query is local
- Use \`query({ ... })\` when the query becomes reusable
- Add \`serve({ queries })\` when you need HTTP routes, OpenAPI, React hooks, or framework handlers
