[**hypequery ClickHouse API**](README.md)

***

# hypequery ClickHouse API

## Classes

### Core

- [ClickHouseConnection](classes/ClickHouseConnection.md)

### Other

- [CrossFilter](classes/CrossFilter.md)
- [JoinRelationships](classes/JoinRelationships.md)

## Interfaces

- [AliasedExpression](interfaces/AliasedExpression.md)
- [JoinPath](interfaces/JoinPath.md)
- [JoinPathOptions](interfaces/JoinPathOptions.md)
- [PageInfo](interfaces/PageInfo.md)
- [PaginatedResult](interfaces/PaginatedResult.md)
- [PaginationOptions](interfaces/PaginationOptions.md)
- [QueryConfig](interfaces/QueryConfig.md)
- [SqlExpression](interfaces/SqlExpression.md)
- [TableSchema](interfaces/TableSchema.md)

## Type Aliases

- [ColumnType](type-aliases/ColumnType.md)
- [DatabaseSchema](type-aliases/DatabaseSchema.md)
- [GroupByExpression](type-aliases/GroupByExpression.md)
- [TableRecord](type-aliases/TableRecord.md)
- [WhereExpression](type-aliases/WhereExpression.md)

## Variables

- [logger](variables/logger.md)

## Functions

- [createQueryBuilder](functions/createQueryBuilder.md)
- [datePart](functions/datePart.md)
- [formatDateTime](functions/formatDateTime.md)
- [raw](functions/raw.md)
- [rawAs](functions/rawAs.md)
- [toDateTime](functions/toDateTime.md)
- [toStartOfInterval](functions/toStartOfInterval.md)
