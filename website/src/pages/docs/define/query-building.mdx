...
Every hypequery project starts with a `defineServe` export that exposes a fluent ClickHouse builder through `ctx.db`. This page shows the most common clauses—`select`, `where`, `join`, `groupBy`, `having`, `orderBy`, `limit`/`offset`, and `toSQL`—using the exact syntax you’ll drop into `analytics/queries.ts`.
