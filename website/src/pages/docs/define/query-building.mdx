...
Every hypequery project starts with an `initServe` export that exposes a fluent ClickHouse builder through `ctx.db`. This page shows the most common clauses—`select`, `where`, `join`, `groupBy`, `having`, `orderBy`, `limit`/`offset`, and `toSQL`—using the exact syntax you’ll drop into `analytics/queries.ts`.
...
> Tip: `serve.queries(...)` keeps contextual typing intact. If you destructure `const { define, queries, query } = initServe(...)`, the rest of the examples work the same way.
...
Calling `await api.execute('tripsQuery')` now returns the same type as `InferQueryResult<typeof api, 'tripsQuery'>`. For inline scripts or devtools, you can lean on the helper to annotate external call sites:
...
<div class="not-prose my-6 rounded-xl border border-blue-100/60 bg-blue-50/80 dark:bg-slate-900/90 dark:border-blue-400/40 p-4 text-sm text-blue-900 dark:text-slate-100">
...
powerUsers: {
  query: ({ ctx }) =>
    ctx.db
      ...
      .groupBy(['user_id'])
      .having((expr) => expr.fn('sum', 'total').gt(5000))
      .having((expr) => expr.fn('count').gte(3)),
}
```

`having` accepts raw strings or predicate callbacks. Each call is AND-ed.
