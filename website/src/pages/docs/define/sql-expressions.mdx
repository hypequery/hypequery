    ...
    tempsInRange: {
      ...
      query: ({ ctx, input }) =>
        ctx.db
          .table('sensors')
          .select(['id', 'reading'])
          .where(raw('reading BETWEEN ? AND ?', [input.min, input.max])),
    },
  },
});
```

- `raw(expression, params?)` accepts any SQL fragment plus optional bindings (positional `?` placeholders).
...
- `rawQuery` returns `unknown[]` by default; pass a type argument when you call it:
  ...
  ```ts
  const rows = await ctx.db.rawQuery<{ day: string; signups: number }>(sql, [input.accountId]);
  ```
