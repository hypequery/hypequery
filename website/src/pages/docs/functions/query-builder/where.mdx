...
Use the builder callback form e.g. `.where(expr => expr.fn(...))` to model advanced
predicates in a type-safe way.
...
Note that Strings are treated as column identifiers for type-safety. Wrap literal values with
`expr.literal(...)` (or `expr.value(...)`) so they’re parameterized instead of interpreted as column names.

```typescript
// String literals must be wrapped
db.table('products')
  .where(expr => expr.fn('equals', 'status', expr.literal('active')))
  .toSQL();
```
