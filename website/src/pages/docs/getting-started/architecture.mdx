...
4. Delivery mechanisms (HTTP adapters, agents) sit at the edge
...
Want concrete examples of those adapters? See the [Serve overview](/docs/serve/overview) for the standalone server, framework handlers, and edge/fetch runtimes powered by the same definitions.
...
5. **(Optional)** Results pass through HTTP adapters
...
**Start building:** [Type Generation](/docs/queries/type-generation) - Generate types from your ClickHouse schema

**Or explore:** [Query Building](/docs/queries/query-building) - Learn the query builder API
