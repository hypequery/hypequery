...
Shipping your first hypequery API now only takes two CLI commands. Install the packages, run `hypequery init` to scaffold everything (env, client, schema, queries), fire up `hypequery dev`, then add a real metric.

### Prereqs

- Node.js 18+
- Access to a ClickHouse instance with credentials saved in `.env`
...
- Install the CLI + TypeScript runner (local dev dependency):

  ```bash
  npm install -D @hypequery/cli tsx
  ```
...
Open `analytics/queries.ts` to see the scaffolded `defineServe` export. You can start editing immediately.
...
export const api = defineServe({
  ...
  queries: {
    activeUsers: {
      ...
      inputSchema: z.object({ limit: z.number().min(1).max(500).default(50) }),
      query: ({ ctx, input }) =>
        ctx.db
          .table('users')
...
- Regenerate schema types on demand with [`hypequery generate`](/docs/define/type-generation)
- Share the API via [HTTP + OpenAPI](/docs/ship/http-openapi)

Once the flow feels solid, add CI checks (`npx hypequery generate` + tests) so schema drift is caught automatically.
