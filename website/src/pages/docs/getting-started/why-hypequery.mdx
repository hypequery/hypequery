...
```typescript
// In your API
const revenue = await api.run('weeklyRevenue', { startDate: '2024-01-01' });
...
// In an AI agent
const result = await api.execute('weeklyRevenue', { input: params });
```
...
- Metadata, validation, and docs live alongside code
- Version control tracks every change
- No debates about "the correct query"
...
- Your backend is written in TypeScript or JavaScript
...
**Continue:** [Installation Options](/docs/getting-started/installation-options) - Choose serve or standalone

**Or jump to:** [Quickstart](/docs/getting-started/quickstart) - Start building (uses serve framework)
