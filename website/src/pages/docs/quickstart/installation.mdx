...
- **Env loader** – `dotenv`, `tsx`, or any framework mechanism that hydrates `process.env`
...
# pnpm
pnpm add -D @hypequery/cli tsx
pnpm add @hypequery/clickhouse @hypequery/serve

# yarn
yarn add -D @hypequery/cli tsx
...
- `tsx` is optional but recommended if you run TypeScript directly (the CLI will use it for `hypequery dev` against `.ts` modules).
...
Load this file before importing `analytics/client.ts`. For plain Node projects add `dotenv/config` to the top of your entrypoint or use `tsx --env-file=.env` during development.
