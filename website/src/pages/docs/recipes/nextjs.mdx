...
- Alias the formatted date (via `formatDateTime(..., { alias: 'day' })`) before grouping/ordering so TypeScript knows `day` exists.
...
```typescript
const api = define({
  middleware: async ({ next, queryName }) => {
    console.log(`[Analytics] ${queryName}`);
    return next();
  },
  queries: queries({ ... }),
});
