...
If your queries file is TypeScript (`.ts`), the CLI will automatically detect and restart with TypeScript support. Make sure `tsx` is installed:
...
| `hypequery create-api-types [queries-file]` | Emit a typed client map (e.g., for `@hypequery/react`) referencing your serve API |
...
If your queries file is TypeScript, the CLI automatically detects and restarts with TypeScript support. Make sure `tsx` is installed (`npm install -D tsx`).
...
```bash
# Without installation
npx @hypequery/cli create-api-types [path/to/queries.ts] [options]

# With installation
npx hypequery create-api-types [path/to/queries.ts] [options]
```

**Options:**
- **`[file]`** – Optional path to your `queries.ts`. Falls back to the same search paths as `hypequery dev` (`analytics/queries.ts`, `src/analytics/queries.ts`, `hypequery.ts`, etc.)
- **`--output <file>`** – Destination for the generated type helper. Defaults to `client.ts` next to your queries file
- **`--name <type>`** – Custom name for the exported type alias (default: `HypequeryApi`)

**What it does:**

The command:
- Loads your serve definition from the queries file
- Enumerates every query key
- Generates a TypeScript type map with `InferQueryInput/InferQueryResult` for each query
- Perfect for frontend React hooks with `@hypequery/react`

Import the resulting type wherever you need an HTTP client—`@hypequery/react`, serverless functions, or SDKs that talk to the deployed API route—without bundling the original `queries.ts` into the client build.
