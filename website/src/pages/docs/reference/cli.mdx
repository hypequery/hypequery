...
```bash
hypequery create-api-types [path/to/queries.ts] [--output analytics/client.ts] [--name HypequeryApi]
```
...
`create-api-types` loads your serve definition, enumerates every query key, and writes a thin TypeScript module that maps `InferQueryInput/InferQueryResult` for each entry. Import the resulting type wherever you need an HTTP client—`@hypequery/react`, serverless functions, or SDKs that talk to the deployed API route—without bundling the original `queries.ts` into the client build.
