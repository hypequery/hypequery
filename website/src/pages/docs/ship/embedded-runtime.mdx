...
5. **Hot reload expectations** – During development the CLI reloads your `queries.ts` file using `tsx`, so edits are picked up automatically. In production you control deployments; keep `defineServe` in a module that is only instantiated once per process to avoid re-registering endpoints.
