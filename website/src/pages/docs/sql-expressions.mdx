...
You can use the `raw`, `rawAs`, or `selectExpr` helpers to include raw SQL expressions in your queries:

```typescript
import { raw, rawAs, selectExpr } from '@hypequery/clickhouse';
...
const query = builder
  ...
  .select([
    ...
    selectExpr('toStartOfWeek(event_time)', 'week')
  ])
  .groupBy(['id', 'user_id'])
  .toSQL();
...
`selectExpr` combines the safety of `raw` with automatic alias handling, so the alias is immediately available to `groupBy`, `orderBy`, and the result type.
...
You can use `raw`, `rawAs`, or `selectExpr` for any custom ClickHouse function:
...
When you need both a custom expression and an alias, `selectExpr` is the most concise option and immediately wires the alias back into the type system:

```typescript
import { selectExpr } from '@hypequery/clickhouse';

const weekly = await builder
  .table('trips')
  .select([
