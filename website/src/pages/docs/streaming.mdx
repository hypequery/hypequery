...
1. **Reader**: The `getReader()` method returns a reader that allows you to consume the stream
2. **Reading Data**: The `read()` method returns a promise with:
   - `done`: boolean indicating if the stream is finished
   - `value`: array of rows from the current batch
3. **Resource Management**: Always use `try/finally` and call `reader.releaseLock()` when done
4. **Error Handling**: Use `try/catch` to handle any errors during stream processing
...
function streamToEvents<T>(stream: ReadableStream<T[]>) {
  const reader = stream.getReader();
...
const emitter = streamToEvents(stream);
